...
class SuperadminController extends Controller
{
    public function index() {
        $berkas = DB::table('daftarberkas')->count();
        $soal = DB::table('soalbaru')->count();
        $user = DB::table('users')
        ->where('role_id', '=','2')
        ->count();
        return view('superadmin.dashboard', compact('berkas','soal','user'));
    }
    ...
    public function indexdataberkasseearch(Request $request) {

        if($request->ajax())
       {

           $output="";
           $users=DB::table('daftarberkas')
           ->where('periode','LIKE','%'.$request->search."%")
           ->orWhere('id','LIKE','%'.$request->search."%")
           ->orWhere('name','LIKE','%'.$request->search."%")
           ->orWhere('asesmenmandiri','LIKE','%'.$request->search."%")
           ->orWhere('schema','LIKE','%'.$request->search."%")
           ->orWhere('tanggal','LIKE','%'.$request->search."%")
           ->orWhere('kartupelajar','LIKE','%'.$request->search."%")
           ->orWhere('rapot','LIKE','%'.$request->search."%")
           ->orWhere('sertifikatpkl','LIKE','%'.$request->search."%")
           ->get();
           if($users)
           {
           $no =0;
               foreach ($users as $key => $user) {
                   $output .= '<tr>' .
                       ...
                       '<td>' . $user->schema . '</td>' .
                       '<td>' . $user->tanggal . '</td>' .
                       '<td>' . $user->asesmenmandiri . '</td>' .
                       '<td>' . $user->name . '</td>' .
                       '<td> <a href="' . asset('storage/berkas/kartupelajar/' . $user->kartupelajar) . '">' .
                        $user->kartupelajar . '</a></td>' .
                        '<td> <a href="' . asset('storage/berkas/rapot/' . $user->rapot) . '">' .
                        $user->rapot . '</a></td>' .
                        '<td> <a href="' . asset('storage/berkas/sertif/' . $user->sertifikatpkl) . '">' .
                        $user->sertifikatpkl . '</a></td>' .
                       '<td>
                       <a href="/daftarberkas/terima/'.$user->id.'"><button type="button" class="btn btn-success">terima</button></a>
                       <br/>
                       <br/>
                       <a href="/daftarberkas/tolak/'.$user->id.'"><button type="button" class="btn btn-danger">tolak</button></a>
                       </td>'.
